...
- [ ] C++ 记录录音时长、WAV 写入耗时、worker 请求总耗时、粘贴耗时
- [ ] Worker 响应保留 `load_ms`、`vad_ms`、`decode_ms`、`punct_ms`
...
- [ ] Worker 启动时输出 Python、sherpa-onnx、numpy 版本
- [ ] Worker 检查模型文件是否存在，并返回明确错误
- [ ] Worker 增加结构化日志：请求 ID、模型 ID、耗时、异常栈
- [ ] C++ 启动 worker 后先 `ping`，失败时显示明确 HUD 错误
- [ ] 避免强制结束主程序后残留 worker 占用 18088 端口

验收标准：
- 模型路径错、端口占用、Python 依赖缺失时都能显示可读错误
- Reload worker 不会留下旧进程

完成后推荐：
- 做 worker 健康检查和自动重启退避，避免无限重启。
...
- [ ] 明确模型不随 exe 打包
- [ ] 写 `INSTALL.md`
- [ ] 写常见问题：Python 依赖、模型目录、端口占用、管理员窗口注入失败
- [ ] 评估是否做 zip 绿色包
