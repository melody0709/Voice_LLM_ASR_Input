...
- v1 不做真正 TSF/IME 内核，先用全局热键、`waveIn` 录音、HUD 显示、剪贴板/SendInput 注入实现稳定可用；WASAPI 是后续低延迟演进方向。
...
## 架构
- `VoiceInput.exe`：C++ Win32 前端，负责托盘、Settings、全局快捷键、`waveIn` 录音、Direct2D HUD、文本注入。
...
- HUD 使用 Direct2D 渲染：
  - 底部居中胶囊窗，当前默认约 56 DIP 高，圆角随高度取半径。
  - DirectWrite 测量文本 DIP，按当前窗口 DPI 转为 Win32 物理像素设置窗口尺寸。
  - RMS 驱动 5 根波形条，权重 `[0.5, 0.8, 1.0, 0.75, 0.55]`。
  - 录音时约 33ms 重绘一次；音量条使用 attack/release 平滑，避免假动画。
...
- 多显示器和 DPI 缩放会影响 HUD 位置和尺寸，必须用当前光标/前台窗口所在显示器计算，并区分 DIP 与物理像素。
